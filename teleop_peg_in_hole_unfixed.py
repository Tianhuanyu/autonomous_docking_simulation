...
pb.connect(
    pb.GUI,
    # options='--mp4=videos/plan_peg_in_hole_unfixed_v3.mp4',
)
pb.resetSimulation()
...
class Controller:
    ...
    def __call__(self, qc, bp, z, f):
        resp = self.response_gain*f_ext_lin
...
class TeleopController:
    ...
    def __init__(self):
        ...
        builder = optas.OptimizationBuilder(T, robots=robot)
        qc = builder.add_parameter('qc', robot.ndof)
        joy = builder.add_parameter('joy', 5)
        q0 = builder.get_model_state(name, 0)
        dq = builder.get_model_state(name, 0, time_deriv=1)
        qF = builder.get_model_state(name, 1)
        builder.enforce_model_limits(name)
        ...
        J = robot.get_geometric_jacobian(ee_link, qc, ee_link)
        J = J[:5, :]
        
        max_vel = optas.diag([0.2, 0.2, 0.2, 0.5, 0.5])  # x,y,z,rx,ry
        dx_goal = max_vel @ joy
        dx = J@dq
        pF = robot.get_global_link_position(ee_link, qF)
        pc = robot.get_global_link_position(ee_link, qc)
        p_goal = pc + time_step * joy[:3]        
        # builder.add_cost_term('eff_pos_goal', optas.sumsqr(pF - p_goal))
        builder.add_cost_term('eff_vel_goal', optas.sumsqr(dx - dx_goal))
        builder.add_cost_term('min_joint_vel', 0.001*optas.sumsqr(dq))
        self.solver = optas.CasADiSolver(builder.build()).setup('ipopt')
        # self.solver = optas.CasADiSolver(builder.build()).setup('sqpmethod')
        self.solution = None
        self.name = name

    def __call__(self, qc, joy):
        if self.solution is not None:
            self.solver.reset_initial_seed(self.solution)
        else:
            self.solver.reset_initial_seed({f'{self.name}/q': optas.horzcat(qc, qc)})
        self.solver.reset_parameters({'qc': qc, 'joy': joy})
        self.solution = self.solver.solve()
        return self.solution[f'{self.name}/q'][:, -1].toarray().flatten()
...
while pb.isConnected():
    ...
    pygame.event.get() # update event loop
    # joystick.init()
    joystick = pygame.joystick.Joystick(0)    
    joy_raw = np.array([joystick.get_axis(i) for i in range(5)])
    # print(joy)
    joy = joy_smooth*joy_raw + (1.-joy_smooth)*joy
    # quit()

    # Call controller for next joint state and update robot
    q = teleop_controller(q, joy)
    robot.cmd(q)
...
# filename = f'f_ext_teleop_{success_msg}_{stamp}.csv'
