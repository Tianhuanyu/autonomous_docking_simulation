...
def f(
    ...
    class Controller:
        ...
        def __init__(self, dt):
            ...
            builder.add_cost_term('dist_to_axis_align', W_TASK_1*dist_to_axis_align_sq)
            ...
            builder.add_cost_term('dist_to_goal', W_TASK_2*task_weight*dist_to_goal_sq)

            # Align end-effector with trocar axis
            builder.add_cost_term('eff_axis_align', W_TASK_3*optas.sumsqr(zF - axis_align))

            # Minimize joint velocity
            W = W_TASK_4*optas.diag([7, 6, 5, 4, 3, 2, 1])
            builder.add_cost_term('min_dq', dq.T@W@dq)

            # Reduce interaction forces
            delta = 0.5
            pR = pc + delta*response
            builder.add_cost_term('reduce_interaction', W_TASK_5*optas.sumsqr(pF - pR))
    ...
    while pb.isConnected():
        ...
        f_ext_lin_nrm_total += f_ext_lin_nrm
    ...
    filename = f'f_ext_bayesopt_{success_msg}_{stamp}.csv'
    ...
    if success:
        ...
        f_ext_lin_nrm_total/=t
        
        reward = - t - 20*f_ext_lin_nrm_total
    else:
        reward = -1000.
...
w_task = {
    k: np.linspace(b[0], b[1], N)
    for k, b in bounds.items()        
}


W_TASK_1, W_TASK_2, W_TASK_3, W_TASK_4, W_TASK_5 = np.meshgrid(
    *[w_task[f'W_TASK_{i}'] for i in range(1, 1+len(bounds))]
)

W_TASK_1 = W_TASK_1.flatten().tolist()
W_TASK_2 = W_TASK_2.flatten().tolist()
W_TASK_3 = W_TASK_3.flatten().tolist()
W_TASK_4 = W_TASK_4.flatten().tolist()
W_TASK_5 = W_TASK_5.flatten().tolist()
...
@dataclass
class DataPoint:
    r: float       # expected reward
    W: np.ndarray  # weight vector
    
idx = 0
for wt1, wt2, wt3, wt4, wt5 in zip(W_TASK_1, W_TASK_2, W_TASK_3, W_TASK_4, W_TASK_5):
    r = 0.    
    for _ in range(repeats):
        r += f(wt1, wt2, wt3, wt4, wt5)
    r/=float(repeats)
    data.append(DataPoint(r, np.array([wt1, wt2, wt3, wt4, wt5])))

dp = max(data, key=lambda dp: dp.r)

print("Optimal weights:", dp.W, ', expected reward', dp.r)
