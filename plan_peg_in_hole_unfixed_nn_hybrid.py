...
from behaviour_cloning import load_model, torch
...
class Controller:
    ...
    def __init__(self, dt):
        ...
        qg = builder.add_parameter('qg', robot.ndof)
        ...
        # Match goal from learned model
        builder.add_cost_term('match_goal', optas.sumsqr(dq - qg))

        # Reduce interaction forces
        delta = 0.5
        pR = pc + delta*response
        builder.add_cost_term('reduce_interaction', 0.05*optas.sumsqr(pF - pR))
    ...
    def __call__(self, qc, bp, z, f, qg):
        ...
        if self.solution is not None:
            ...
            self.solver.reset_initial_seed({f'{self.name}/q': optas.horzcat(qc, qg)})
        self.solver.reset_parameters({'qc': qc, 'box_pos': bp, 'axis_align': z, 'response': resp, 'qg': qg})
        t0 = perf_counter()
...
while pb.isConnected():
    ...
    state_np = np.concatenate((q, box_base_position, box_base_orientation))
    state = torch.from_numpy(state_np)
    action = model(state)
    dq = action.detach().numpy().flatten()
    qg = q + time_step*dq
    # print("--------------------")
    # print("action=", )
    # print("--------------------")
    ...
    q = controller(q, box_base_position, goal_z, f_ext_lin, dq)
    robot.cmd(q)
...
filename = f'f_ext_nn_hybrid_{success_msg}_{stamp}.csv'
