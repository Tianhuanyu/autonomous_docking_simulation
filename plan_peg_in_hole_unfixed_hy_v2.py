...
class Controller:
    ...
    def __init__(self, dt):
        ...
        delta = 0.2
        cvf = Rc.T @ delta @ response
        diffp = (I3() - nf @ nf.T) @ (p - cvf - p_box_pos)
        ...
        # W_p1 = optas.diag([1e1, 1e1, 0.0])
        # error=diffpp.T @ diffpp
        # print("error = {0}".format(error.size()))
        diffpp = optas.sumsqr((I3() - nf @ nf.T) @ p_box_pos)
        ff = 0.1 * optas.np.exp(-1.0 * diffpp)
        diffFl = nf @ nf.T @ (cvf + ff * optas.DM([0.0, 0.0, 1.0]) - optas.diag([2e1, 2e1, 2e1]) @ Rc.T @dp[:3])
